...
#region Quick Setup
## Helper function that connects the [code]on_handle_drag[/code] signal to the given function and sets the required function names for the gizmo's undo and redo actions. Used to simplify setting up the gizmo after creating it.[br]
## undo callback arguement Dict: [code]{ oldSize : Vector2, newSize : Vector2, newOffset : Vector2 }[/code][br]
## redo callback arguement Dict: [code]{ oldSize : Vector2, newSize : Vector2, newOffset : Vector2 }[/code][br]
func quick_setup( handleDragFunc : Callable, undoFunc : String, redoFunc : String, givenActionName : String = 'Gizmo Action' ) -> void:
    on_handle_drag.connect( handleDragFunc )
    onUndo = undoFunc
    onRedo = redoFunc
    actionName = givenActionName

## Helper function used to quickly set the handle display variables all at once.
func quick_set_handles( TL : bool, TR : bool, BL : bool, BR : bool ) -> void:
    handleTL = TL
    handleTR = TR
    handleBL = BL
    handleBR = BR
...
@export var rect : Rect2 = Rect2( Vector2.ZERO, Vector2( 30.0, 30.0 ) ) :
    set(inp):
        ...
        on_var_changed()
        _position_gizmos()
        _modifiedRect.position = rect.position
        _modifiedRect.size = rect.size
...
## Emitted while a handle is in the draw function but before it is actually drawn.
signal pre_handle_draw( handleCanvas : Gizmo_Canvas, handle : J_Gizmo2D_Handle, this_gizmo : J_Gizmo2D_Rect )
#endregion
...
## Generates and sets up all of this gizmo's the handle subgizmos based on the handle display variables. Automatically called by the quick_set_handles function.
func setup_gizmos( ) -> void:
    ...
    for gizmoIndex in range(4):
        ...
        newGizmo.pre_draw.connect( pre_handle_draw.emit.bind( self ) )
        newGizmo.overrideUndoRedo = '_blank_handle_undoredo'

        # newGizmo.referenceNode = get_ref_node()
...
func _position_gizmos() -> void:
    for gizmo in gizmos:
        ...
        match gizmo.get_meta( 'gizmo_id' ):
            'TL':
                gizmo.position = rect.position
            'TR':
                gizmo.position = Vector2( rect.position.x+rect.size.x, rect.position.y )
            'BL':
                gizmo.position = Vector2( rect.position.x, rect.position.y+rect.size.y )
            'BR':
                gizmo.position = Vector2( rect.position.x+rect.size.x, rect.position.y+rect.size.y )
...
## - POSITIONAL : 0 - The handle both scales and repositions the _modifiedRect on the axis ( emitting an on_rect_change signal with both a real offset and size vector on that axis ).[br]
## - SIZE : 1 - The handle only scales the _modifiedRect on the axis ( emitting an on_rect_change signal with a zero offset vector on that axis ).[br]
## - LOCKED : 2 - The handle is unable to modify the _modifiedRect on the axis ( emitting an on_rect_change signal with a zero size vector on that axis ).
enum HANDLE_TYPE_ENUM { POSITIONAL, SIZE, LOCKED }
## Handles the dragging of the handle subgizmos, modifying the gizmo's rect based on the type of handle being dragged and emitting the appropriate signals. Automatically connected to the handle subgizmos when they are generated in _setup_gizmos.
func _on_handle_drag( dragPos : Vector2, dragVector : Vector2, handle : J_Gizmo2D_Handle ) -> void:
...
func _calc_handle_mod( newRect : Rect2, handleType : HANDLE_TYPE_ENUM, dragPos : Vector2, axis : int ) -> Rect2:
    match handleType:
        HANDLE_TYPE_ENUM.POSITIONAL:
            newRect.size[ axis ] += _modifiedRect.position[ axis ] - dragPos[ axis ]
            newRect.position[ axis ] -= _modifiedRect.position[ axis ] - dragPos[ axis ]

            if newRect.size[axis] <= 0:
                newRect.position[ axis ] += newRect.size[ axis ]
                newRect.size[ axis ] = 0
                
        HANDLE_TYPE_ENUM.SIZE:
            newRect.size[ axis ] += dragPos[ axis ] - ( _modifiedRect.position[ axis ] + _modifiedRect.size[ axis ])
...
func draw_gizmo() -> void:
    ...
    if drawRect:
        var drawRect : Rect2 = rect
        drawRect.size *= get_ref_node().get_global_transform().get_scale()
        canvas.draw_rect( drawRect, rectColor, false, rectThickness/get_viewport_transform().x.x )
