...
signal on_drag( dragPos : Vector2, this_gizmo : J_Gizmo2D_Handle )
...
## Emitted after the gizmo is drawn but before it leaves the draw function, allowing for additional custom drawing on top of the gizmo.
signal post_draw( gizmo_canvas : Gizmo_Canvas, this_gizmo : J_Gizmo2D_Handle )
...
func _on_canvas_gui_input(event) -> bool:
    ...
    match _state:
        ...
        STATES.GRABBED:
            mouseInside = true
            if event is InputEventMouseMotion:
                on_drag.emit( get_local_mouse_position() - (gizmoOffsetVector*offset)/get_viewport_transform().x.x, self )
                refresh_canvas()
                return( true )
...
func draw_gizmo() -> void:
    ...
    _displayPosition = position + gizmoOffsetVector*offset/viewportScale
